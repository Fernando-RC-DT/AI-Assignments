...
prototxt_path = "resources/vision_ai/deploy.prototxt"
model_path = "resources/vision_ai/res10_300x300_ssd_iter_140000.caffemodel"
...
def detect_faces(photo):
    ...
    LoggingUtil.display_text("Using Haar cascades for face detection")
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
    faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
    if faces == ():
        LoggingUtil.display_text("No faces detected.")
...
def search_similar_images(photo):
    ...
    if search:
        ...
        LoggingUtil.display_text(f'Results: {json.dumps(results, indent=2)}')
    ...
    LoggingUtil.display_text(f"Search results: {search_results}")
    return search_results

def detect_faces_dnn(photo):
    LoggingUtil.display_text("Using DNN for face detection")
...
def summarize_title_and_links(titles_and_links_result):
    ...
    LoggingUtil.display_text(f"titles_and_links_result: {titles_and_links_result}")
    ...
    LoggingUtil.display_text(f"Response: {response}")
    return response
...
if __name__ == "__main__":
    ...
    for result in search_results:
        LoggingUtil.display_text(f"Title: {result['title']}, Link: {result['link']}")
